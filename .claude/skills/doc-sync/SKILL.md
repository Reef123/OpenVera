...
| `.curate-running` | /curate at start (content: `RUNNING <ISO-ts>`, not a bare touch) | /curate at end, LAST — after the commit + date-stamp land | Same 60-min TTL for `mark-dirty.py`'s skip check; `session-start.py` reads the `RUNNING <ts>` line and surfaces a crash notice before removing it unconditionally at boot (any leftover lock = a crash — new session, nothing legitimately still running) |
...
- **Top projects:** ≤5 rows, Stage + Pulse (🟢 moved <7 days / 🟡 blocked-on-you / 🔴 stale 14+ days and not parked). **Delta-only, always** — derive this list from THIS SESSION's touched projects (git diff + anything mentioned in conversation), never a full walk of `{projects_dir}/*`. The full project census (every project, tiered hot/cold) is `/curate`'s weekly job (curate/SKILL.md §6.7) — doc-sync never re-does it.
...
- **Curate health line — informational only, never a blocked slot.** One line: `Curate: last ran YYYY-MM-DD (Nd ago) · M open flags (oldest: K runs)`. Read `.claude/last-curate-date` for the date/age. For the flag counts, read `vera-system/memory/curate-flags.md` at render time and count rows: `M` = rows with `Status` starting `open` (case-insensitive); `K` = the highest `Runs survived` among those open rows (`oldest: 0 runs` if the ledger is empty). Don't pull flag counts from telemetry — `telemetry.py`'s schema has no flag fields; the ledger is the source of truth, counted fresh every render. Prefix with ⚠ when `Nd` > 7, OR `.claude/.curate-running` exists (stale lock), OR any open row's `Runs survived` ≥ 3. This line never blocks or claims a slot itself — it's a status readout; the park-or-kill ask (if any) is a separate boot-tier item per `vera-system/CLAUDE.md`.
